...
class AbyssRuneCrafter(OSRSBot):
    ...
    def set_directions(self):
        ...
        self.to_abyss_steps = [
            ...
            StairsStep(start=Point(3089,3523), end=self.mage_point, description="tele to abyss", mouseover_text="Teleport", color=self.cp.hsv.CYAN_MARK),
        ]
        self.to_bank_steps = [
    ...
    def resupply(self) -> bool:
        ...
        if rect := self.find_sprite(win=self.win.game_view, png="pure-essence.png", folder="items"):
            self.mouse.move_to(rect.random_point())
            self.mouse.click()
            self.sleep()
